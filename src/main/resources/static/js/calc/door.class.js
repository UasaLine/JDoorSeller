class Door {
    ...
    static createСolor(container,config){
        ...
        $("<img>")
            ...
            .attr(
                "style",
                "width:" + config.width + "px; height:" + (config.doorFanlightHeight + config.height) + "px;"
            )
            .appendTo(container);
    }
    static createTrims(container, config, door){
        ...
        if (door.leftDoorTrim == 0){
            $("<div>")
                ...
                .attr(
                    "style",
                    "width:3.5%; height:" + (config.doorFanlightHeight + config.height) + "px;"
                )
                .appendTo(container);
        }
    ...
    static createFanlight(container, config,side){
        if (config.doorFanlightHeight > 0) {
            ...
            $("<div>")
                ...
                .attr(
                    "style",
                    "width:" + config.width + "px; height:" + config.doorFanlightHeight + "px;"
                )
                .appendTo(container);
    ...
    static createHandle(containerLeaf,config,door,side){
        if(side === "L"){
            if (door.furnitureKit != null && door.furnitureKit.handle != null) {
                if (door.sideDoorOpen == "LEFT") {
                    $("<img>")
                        .attr("class", "handle_images left_side" + config.leafCount + "_handle")
                        .attr("src", "" + door.furnitureKit.handle.sketchPathFirst)
                        .appendTo(containerLeaf);
                } else {
                    $("<img>")
                        .attr("class", "handle_images rigth_side" + config.leafCount + "_handle mirrorX")
                        .attr("src", "" + door.furnitureKit.handle.sketchPathFirst)
                        .appendTo(containerLeaf);
                }
            }
