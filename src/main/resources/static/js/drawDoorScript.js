...
function drawObject(door,i){
        ...
        if	(!!color && !!width && !!height){
            ...
            if (doorFanlightHeight>0) {
                ...
                //draw fanlight
                $('<div>').attr('class', 'fanlight')
                    .attr('id', 'fanlightL' + i)
                    .attr('style', 'width:' + width + 'px; height:' + doorFanlightHeight + 'px;').appendTo('#picture_doorL' + i);

                $('<img>').attr('class', 'opening_side_images')
                    .attr('src', 'images/Door/fanlight_in.png')
                    .attr('style', 'width:' + width + 'px; height:' + doorFanlightHeight + 'px;').appendTo('#fanlightL' + i);
            }

            //draw Leaf
            $('<div>').attr('class','Leaf')
                .attr('id','LeafL'+i)
                .attr('style','width:'+width+'px; height:'+height+'px;').appendTo('#picture_doorL'+i);

                $('<img>').attr('class','opening_side_images')
                    .attr('src','images/Door/'+sideOpeningL+'.png')
                    .attr('style','width:'+width+'px; height:'+height+'px;').appendTo('#LeafL'+i);

                //draw closer
                if(door.furnitureKit!=null && door.furnitureKit.closer!=null){
                    $('<img>').attr('class','closer_images')
                        .attr('src',''+door.furnitureKit.closer.sketchPathFirst).appendTo('#LeafL'+i);
                }

                //draw handle
                if(door.furnitureKit!=null && door.furnitureKit.handle!=null) {
                    if(door.sideDoorOpen == "LEFT" ){
                        $('<img>').attr('class','handle_images left_side'+leafCount+'_handle')
                            .attr('src',''+door.furnitureKit.handle.sketchPathSecond).appendTo('#LeafL'+i);
                    }
                    else {
                        $('<img>').attr('class','handle_images rigth_side'+leafCount+'_handle')
                            .attr('src',''+door.furnitureKit.handle.sketchPathFirst).appendTo('#LeafL'+i);
                    }
                }

                //draw doorstep
                if(door.furnitureKit!=null && door.stainlessSteelDoorstep!=null && door.stainlessSteelDoorstep!=0) {
                $('<img>').attr('class','stainlessSteelDoorstepL_images')
                    .attr('src','images/findings/door handles sketch/000000001_R.png').appendTo('#LeafL'+i);
                }
        ...
        if	(!!color && !!width  && !!height){
            ...
            $('<img>').attr('class','color_door').attr('src','images/Door/AColor1/'+color+'.jpg')
                .attr('style','width:'+width+'px; height:'+(doorFanlightHeight+height)+'px;').appendTo('#picture_doorR'+i);
            ...
            if (doorFanlightHeight>0) {
                ...
                $('<div>').attr('class', 'fanlight')
                    .attr('id', 'fanlightR' + i)
                    .attr('style', 'width:' + width + 'px; height:' + doorFanlightHeight + 'px;').appendTo('#picture_doorR' + i);

                    $('<img>').attr('class', 'opening_side_images')
                        .attr('src', 'images/Door/fanlight_out.png')
                        .attr('style', 'width:' + width + 'px; height:' + doorFanlightHeight + 'px;').appendTo('#fanlightR' + i);
            }

            //draw Leaf
            $('<div>').attr('class','Leaf')
                .attr('id','LeafR'+i)
                .attr('style','width:'+width+'px; height:'+height+'px;').appendTo('#picture_doorR'+i);

                $('<img>').attr('class','opening_side_images')
                    .attr('src','images/Door/'+sideOpeningR+'.png')
                    .attr('style','width:'+width+'px; height:'+height+'px;').appendTo('#LeafR'+i);

                //draw handle
                if(door.furnitureKit!=null && door.furnitureKit.handle!=null) {
                    if(door.sideDoorOpen == "LEFT" ){
                        $('<img>').attr('class','handle_images rigth_side'+leafCount+'_handle')
                            .attr('src',''+door.furnitureKit.handle.sketchPathFirst).appendTo('#LeafR'+i);
                    }
                    else {
                        $('<img>').attr('class','handle_images left_side'+leafCount+'_handle')
                            .attr('src',''+door.furnitureKit.handle.sketchPathSecond).appendTo('#LeafR'+i);
                    }
                }
                ...
                //draw doorstep
                if(door.furnitureKit!=null && door.stainlessSteelDoorstep!=null && door.stainlessSteelDoorstep!=0) {
                    $('<img>').attr('class','stainlessSteelDoorstepR_images')
                        .attr('src','images/findings/door handles sketch/000000001_R.png').appendTo('#LeafR'+i);
                }
